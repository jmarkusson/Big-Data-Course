...
def count_words_in_file(filename_queue: mp.Queue,wordcount_queue: mp.Queue,batch_size: int):
    """
    ...
    while True:
        ...
        # Checking if the filename queue is empty
        if filename is None:
            wordcount_queue.put(counts)
            wordcount_queue.put(None) # Signal that the worker is done
            break

        # Regular counting logic as before
        ...
        # Checking if batch size is met, then sending, reset process
        file_counter += 1
