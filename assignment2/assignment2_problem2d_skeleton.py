...
if __name__ == '__main__':
    ...
    time_arg = time.perf_counter() - arg_block_start # Time recorded for argument block

    read_block_start = time.perf_counter()
    files = [get_file(fn) for fn in get_filenames(path)] # Parallizable 
    time_read = time.perf_counter() - read_block_start # Time recorded for appending file paths

    count_block_start = time.perf_counter()
    with mp.Pool(processes=num_workers) as pool:
        file_counts = pool.map(count_words_in_file, files, chunksize=batch_size)
    time_count = time.perf_counter() - count_block_start # Time recorded for counting word occourences and appending dictionary
    ...
    time_total = time.perf_counter() - total_start # Time recorded for the total main program
